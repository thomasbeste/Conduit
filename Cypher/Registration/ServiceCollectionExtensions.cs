...
/// <summary>
/// Extension methods for registering Cypher services with the DI container.
/// </summary>
public static class ServiceCollectionExtensions
{
    extension(IServiceCollection services)
    {
        public IServiceCollection AddCypher(Action<CypherConfiguration> configure)
        {
            var config = new CypherConfiguration();
            configure(config);

            return services.AddCypher(config);
        }

        public IServiceCollection AddCypher(CypherConfiguration config)
        {
    ...
    /// <param name="assemblies">Assemblies to scan for request types. Should match assemblies passed to <see cref="CypherConfiguration.RegisterServicesFromAssembly"/>.</param>
    ...
    /// </code>
    /// </remarks>
    public static void ValidateCypherRegistrations(this IServiceProvider serviceProvider, params Assembly[] assemblies)
    {
        ...
        if (errors.Count > 0)
        {
            throw new InvalidOperationException(
                $"Cypher handler registration validation failed:\n{string.Join("\n", errors.Select(e => $"  - {e}"))}");
        }
    }
}
